 ...
 *				requirements.
 */
public class ItalianLoader {
	@SuppressWarnings("resource")
	public static void main(String[] args){
		try{
			//Establish a connection to ServiceBroker & print over stream confirmation
		//	JOptionPane.showMessageDialog(new JFrame(), "[ItalianLoader] ItalianLoader loaded!");
			...
			while(true){
		//		JOptionPane.showMessageDialog(new JFrame(), "[ItalianLoader]Connecting...");
				translation = serverSocket.accept();
				ClientHandler ch = new ClientHandler(translation);
				ch.run();
			}
		}catch(Exception e){
			JOptionPane.showMessageDialog(new JFrame(), "[ItalianLoader] Cannot open socket 9092! \n" + e.getMessage());
			e.printStackTrace();
			System.exit(0);
		}
