 ...
 * to the ItalianLoader.jar and loads the translations
 * from the french.txt file to pass back to the 
	...
	public void run(){
		...
		try{
			...
			try {
				...
				JOptionPane.showMessageDialog(new JFrame(), "[ItalianLoader] ERROR: \n" + e.getMessage());
				out.print("[ERROR]" + e.getMessage());
				e.printStackTrace();
				System.exit(0);
			}
		}catch(Exception e){
			JOptionPane.showMessageDialog(new JFrame(), "[ItalianLoader] Can't open output stream!");
			e.printStackTrace();
			System.exit(0);
		}
	}
