 ...
 *				requirements.
 */
public class FrenchLoader {
	@SuppressWarnings("resource")
	public static void main(String[] args){
		try{
			//Establish a connection to ServiceBroker & print over stream confirmation
		//	JOptionPane.showMessageDialog(new JFrame(), "[FrenchLoader] FrenchLoader loaded!");
			...
			while(true){
		//		JOptionPane.showMessageDialog(new JFrame(), "[FrenchLoader]Connecting...");
				translation = serverSocket.accept();
				ClientHandler ch = new ClientHandler(translation);
				ch.run();
			}
		}catch(Exception e){
			JOptionPane.showMessageDialog(new JFrame(), "[FrenchLoader] Cannot open socket 9092! \n" + e.getMessage());
			e.printStackTrace();
			System.exit(0);
		}
